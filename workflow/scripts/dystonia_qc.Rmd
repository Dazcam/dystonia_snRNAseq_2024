...
+ May need an additional UMI filter
+ Need more stringent Mito filter
+ Are there still doublets here?
...
+ Some evidence of batch effects 
+ Get cell counts per cluster
+ PC threshold probs between 20-30 for all regions except FCX
...
+ Some evidence of batch effects 
+ Similar scenario for CCA, RPCA and FastMNN
...
+ Nice distinction between cell types using general gene set
+ Need to refine gene set for each region
+ Need to refine cluster numbers and cluster IDs (and colour Vln plots by general cell type)
...
+ Will improve (a bit) when cluster IDs are determined
...
+ How can we aggregate the dystonia data to more easily see patterns?
+ PCA?
+ Heatmap?
+ WGCNA?
...
+ Only showing adult for now
+ Try `AggregateExpression()`

***

```{r heatmap_all, results='asis', fig.dim = c(10,10), echo=FALSE}
...
```{r pca_all, results='asis', fig.dim = c(10,10), echo=FALSE}
plot_grid(pca_plot, pca_load_plot, scree_plot, ncol = 2, align = 'hvlr')
```

***

```{r pca_facet, results='asis', fig.dim = c(10,10), echo=FALSE}
pairs_plot
```
