...
regions <- c('fcx', 'str', 'cer', 'sng')
fcx_anns <- c('A13', 'A14', 'A25', 'A32', 'A44-A45', 'A46', 'FI', 'M1C')
str_anns <- c('CaB', 'Pu')
cer_anns <- c('CBL', 'CBV', 'CbDN')
sng_anns <- c('SN')
all_anns <- c(fcx_anns, str_anns, cer_anns, sng_anns)
...
# Note SNG added in review, fewer cells / samples but used sketch for consistency
sketch_num <- dplyr::case_when(
  ...
  region == "sng" ~ 4150) # 10 samples (10 * 4150 = 41500 cells) # Added in review
